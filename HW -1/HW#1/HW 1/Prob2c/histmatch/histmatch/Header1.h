...
unsigned char*** extend(unsigned char ***inputimg, int h,int w,int n)
{
	...
	for(int i=0;i<(n>>1);i++)
	{
		...
		for(int k=0;k<h;k++)
		{
			...
			extended_img[k+(n>>1)][i+w+(n>>1)][0]=inputimg[k][0][0];
			extended_img[k+(n>>1)][i+w+(n>>1)][1]=inputimg[k][0][1];
			extended_img[k+(n>>1)][i+w+(n>>1)][2]=inputimg[k][0][2];
