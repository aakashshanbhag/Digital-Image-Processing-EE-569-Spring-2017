...
int main(int argc, char* argv[])
{
	...
	if (argc < 3)
	{
		...
		cout<<"program_name.exe hillary_image.raw output_image.raw piece.raw "<<endl ;
		return 0;
	}
	...
	unsigned char *Imagedata;// hillary or trump
	unsigned char *Imagedata1;// piece image
	unsigned char *Imagedata2;// output image
	

	//	Conversion To 1D Array. 
	Imagedata= new unsigned char [Size_w*Size_h*BytesPerPixel];// hillary trump
	...
	//	Reading input file.
	fileread(file,argv[1],Imagedata,Size_h,Size_w,BytesPerPixel);// hillary read
...
//------------------------------------------------------------------------------------------------------	
	...
	unsigned char ***Imgdata=NULL;	// Hillary
...
//------------------------------------------------------------------------------------------------------		
	...
	for ( i=0;i<piece_h;i++)
	{
		for( j=10;j< piece_w/2;j++)
		{
			...
			else 
			{
				...
				{ 
					...
					jleft=j;// 1st instance set to compare with the following values.
					ibottom=i;
					jright=j;
	 ...
	 cout<<"Locations of Hillary piece image are as follows:"<<endl;
	...
	double U1,V1,U2,V2,U3,V3,U4,V4;
			...
			U2=img2cartx(jright);
			U3=img2cartx(jleft);
			U4=img2cartx(jbottom);

		V1=img2carty(itop,piece_h);
		V2=img2carty(iright,piece_h);
		V3=img2carty(ileft,piece_h);
		V4=img2carty(ibottom,piece_h);

	cout<<"topvalues Cartesian:" <<U1<<" "<<V1<< endl;
	cout<<"rightvalues Cartesian:" <<U2<<" "<<V2<< endl;
	cout<<"leftvalues Cartesian:" <<U3<<" "<<V3<< endl;
	cout<<"bottomvalues Cartesian:" <<U4<<" "<<V4<< endl;
	cout<< "  "<<endl;
//------------------------------------------------------------------------------------------------------	
	// Calculation of the height and width of the puzzle piece by euclidean distance formula
	...
	double scalefactorx=(100)/(wdistance);
	double scalefactory=(100)/(hdistance);


	// find the angle of the rotation
	
	double divfactor=(V3-V1)/(1.0*(U3-U1));
	double angle=(3.14159/2.0)-atan(divfactor);
...
//------------------------------------------------------------------------------------------------------		
	...
	for(int i=0;i<piece_h;i++)
	{
		...
		double x=img2cartx(j)+U3-floor(wdistance/2.0)-1;
		double y=img2carty(i,piece_h)+V3-floor(hdistance/2.0)-22;
...
//------------------------------------------------------------------------------------------------------	
	...
	// First coordinate of the eye image to remap.
	int flag2=0;
	for(int i=0;i<piece_h;i++)
	{
		for(int j=0;j<piece_w ;j++)
	...
	// Final output with the missing piece inserted.
	...
	for(int i=0;i<Size_h;i++)
	{
		for(int j=0;j<Size_w ;j++)
		{
			...
			{
				img1[i][j][0]=Imgdata2[yeye+i-yhill+1][xeye+j-xhill+1][0];
				img1[i][j][1]=Imgdata2[yeye+i-yhill+1][xeye+j-xhill+1][1];
				img1[i][j][2]=Imgdata2[yeye+i-yhill+1][xeye+j-xhill+1][2];
			}
			else
			{
