...
{float psnrb;
	...
	for(int i=0;i<Size_h;i++)
	{
		for(int j=0;j<Size_w;j++)
		{
			msrb=msrb+(pow(1.0*(Imagedatac[i*3*Size_w+j+2]-1.0*Imagedata[3*i*Size_w+j+2]),2.0));
			}
...
double img2cartx(int x)
{double X;
X=x+0.5;
return X;}

double img2carty(int x, int height)
{double Y;
Y=height-0.5-x;
return Y;}


double cart2imgx(double x)
{double X;
X=x-0.5;
return X;}


double cart2imgy(double x, int height)
{double Y;
Y=height-0.5-x;
return Y;}
